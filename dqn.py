...
def nature_cnn(observation_space, depths = (32, 64, 64), final_layer=512):
    ...
    cnn = nn.Sequential(
        ...
        nn.ReLU(),
        nn.Conv2d(n_input_channels, depths[1], kernel_size = 4, stride = 2),
        nn.ReLU(),
        nn.Conv2d(n_input_channels, depths[2], kernel_size = 3, stride = 1),
        nn.ReLU(),
        nn.Flatten())
...
class Network(nn.Module):
    ...
    def act(self, obses, epsilon):
        ...
        for i in range(len(actions)):
            rnd_samples = random.random()
            if rnd_samples <= epsilon:
...
#env =  SubproVecEnv([make_env for _ in NUM_ENVS])
...
for _ in range(MIN_REPLAY_SIZE):
    ...
    for obs, action, rew, done, new_obs in zip(obses, actions, rews, dones, new_obses):
        transition = (obs, actions, rew, done, new_obs)
